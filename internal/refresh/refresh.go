...
func Render(rows []Row, generatedAt string, today time.Time) string {
	...
	fmt.Fprintf(&b, "---\ntitle: Atlas\ngenerated_at: %s\n---\n\n# Atlas\n\n", generatedAt)
	fmt.Fprintf(&b, "%s · refreshed %s\n\n", strings.Join(parts, " · "), stamp.Local().Format("2006-01-02 15:04"))
	b.WriteString("This page is generated by `claude-atlas refresh`. Edit intent in each node's\n`node.md`; everything else here is derived and will be overwritten.\n\n")
	b.WriteString("| Heat | Node | Priority | State | Idle | Pages | Threads | Unfinished | |\n")
	b.WriteString("|:--|:--|:--|:--|--:|--:|--:|--:|:--|\n")
	...
	for _, r := range sorted {
		...
		if v := r.Node.VaultPath(); v != "" {
			link = fmt.Sprintf("[open](%s)", obsidian.OpenURI(v))
		}
		fmt.Fprintf(&b, "| %s | [[tree/%s/node\\|%s]] | %s | %s | %s | %s | %d | %s | %s |\n",
			heat, r.Node.Rel, label, r.Node.Priority, r.Node.State, idle(r.State),
			intOr(r.State.Pages, "—"), len(r.State.OpenThreads), unfinishedTotal(r.State.Unfinished), link)
	}
	var flagged []string
	for _, r := range rows {
		for _, note := range Signals(r.Node, r.State, today) {
			flagged = append(flagged, fmt.Sprintf("- **%s** — %s", r.Node.Rel, note))
		}
	}
	if len(flagged) > 0 {
	...
	for _, r := range rows {
		...
		var line string
		if v := r.Node.VaultPath(); v != "" {
			line = fmt.Sprintf("`%s` · [open](%s)", home.Display(v), obsidian.OpenURI(v))
		} else {
			line = fmt.Sprintf("cluster · %s vaults", intOr(r.State.Leaves, "0"))
		}
		line += fmt.Sprintf(" · [[tree/%s/node|node]]", r.Node.Rel)
		if r.State.LastTouched != "" {
			line += fmt.Sprintf(" · last touched %s (%s)", r.State.LastTouched, idle(r.State))
		}
		if r.State.Pages != nil {
			line += fmt.Sprintf(" · %d pages", *r.State.Pages)
		}
		b.WriteString(line + "\n")
		if r.Node.Purpose != "" {
			b.WriteString("\n" + strings.TrimSpace(r.Node.Purpose) + "\n")
		}
		if r.Node.DefinitionOfDone != "" {
			b.WriteString("\n**Done when:** " + strings.TrimSpace(r.Node.DefinitionOfDone) + "\n")
		}
		...
		if u.EmptySections != nil || u.SeedPages != nil || u.DeadLinks != nil {
			...
			for _, kv := range []struct {
				...
				if kv.v != nil {
					bits = append(bits, fmt.Sprintf("%s %d", kv.k, *kv.v))
				}
			}
			b.WriteString("\n**Unfinished** " + strings.Join(bits, " · ") + "\n")
		}
		if len(r.Node.Repos) > 0 {
			b.WriteString("\n**Repos** " + strings.Join(r.Node.Repos, " · ") + "\n")
		}
	}
	return b.String()
}
...
// Run refreshes every node and writes Atlas.md; it returns the page path and rows.
func Run(cfg *home.Config, p *product.Product, today time.Time) (string, []Row, error) {
	...
	page := filepath.Join(cfg.AtlasVault, "Atlas.md")
