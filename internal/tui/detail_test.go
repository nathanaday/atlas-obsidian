...
func TestBoxLinesShowBothHalves(t *testing.T) {
	items := sample()
	pr := strings.Join(boxLines(items[0].Entry, 60), "\n")
	for _, want := range []string{"webapp", "/code/webapp", "the webapp work", "4 pages", "3 threads open", "phase: Alarm quality", "1 in inbox", "touched today"} {
		if !strings.Contains(pr, want) {
			t.Errorf("project box misses %q:\n%s", want, pr)
		}
	}
	problem := strings.Join(boxLines(items[3].Entry, 60), "\n")
	if !strings.Contains(problem, "✗ gateway") || !strings.Contains(problem, "not found") {
		t.Errorf("problem box:\n%s", problem)
	}
	fresh := registry.Entry{Name: "new", Path: "/code/new"}
...
func TestDetailLinesForAProject(t *testing.T) {
	...
	for _, want := range []string{"Path", "/code/webapp", "Mode", "generic", "Git", "main · 2 uncommitted · last commit 2026-09-16", "Described", "2 commits behind", "Wiki", "4 pages", "Hot topics", "Threads", "3 open", "Phases", "Alarm quality → Launch", "[plan] Filter vehicle false alarms", "Alarm quality", "[stub] Blocked one", "blocked", "stale", "Signal", "1 blocked thread"} {
