...
**Goal:** Replace the folder tree in `atlas-obsidian view` with one tab per kind, a box per vault, a dashed connector from every project to the knowledge bases it mounts, and expansion in place instead of a detail screen.
...
func TestBoxLinesByKind(t *testing.T) {
	...
	bad := registry.Entry{Path: "/v/old-notes", Error: "v1 vault; run atlas-obsidian adopt", Reason: registry.ReasonV1}
	if lines := boxLines(bad); lines[0] != "old-notes" || lines[1] != "v1 vault; run atlas-obsidian adopt" {
...
func TestDetailLinesByKind(t *testing.T) {
	...
	bad := registry.Entry{Path: "/v/old", Error: "v1 vault; run atlas-obsidian adopt /v/old --as knowledge|project", Reason: registry.ReasonV1}
...
func problemFix(e registry.Entry) string {
	...
	case registry.ReasonV1:
		return "press a to adopt it, or run atlas-obsidian adopt " + path + " --as project|knowledge"
	case registry.ReasonMissing:
		return "press e then r to forget it, or run atlas-obsidian remove " + path
...
var empties = map[tab]string{
	tabProjects:  "no projects yet; press n to make one, or run `atlas-obsidian new-project NAME`",
	tabKnowledge: "no knowledge bases yet; press N to make one, or run `atlas-obsidian new-knowledge NAME`",
}
...
const (
	v1Error = "v1 vault; run atlas-obsidian adopt /v/old-notes --as knowledge|project"
	v1Start = "v1 vault; run atlas-obsidian adopt"
)
...
func TestProblemsTabAdoptsAndExplains(t *testing.T) {
	...
	gone := sample()
	gone[5].Entry.Reason, gone[5].Entry.Error = registry.ReasonMissing, "not found; run atlas-obsidian remove /v/old-notes to forget it"
...
Run `make build && ./build/atlas-obsidian view` in a terminal at 80 and at 120 columns: the tabs, the connectors, Enter, Esc, ←→, `T`, and `q`. Fix what looks wrong before the commit. Report what you saw in the report file.
...
```
`atlas-obsidian` with no command, or `atlas-obsidian view`, is one screen with a
...
Set `"version": "1.1.0"` in `.claude-plugin/plugin.json` and in both places in `.claude-plugin/marketplace.json`. Run `make build && ./build/atlas-obsidian version 2>/dev/null || ./build/atlas-obsidian --version` to see the binary report 1.1.0 (the Makefile reads plugin.json).
