...
Add `"github.com/nathanaday/atlas-obsidian/internal/txn"` to the imports, and append:
...
- Produces: `atlas-obsidian stub VAULT [TITLE...] [--type T]`.
...
func (e *env) stub(args []string) (int, error) {
	...
	if len(positional) < 1 {
		return 2, errors.New("usage: atlas-obsidian stub VAULT [TITLE...] [--type T]")
	}
...
atlas-obsidian stub VAULT stubs every wanted page; titles and --type narrow it.
...
In `internal/hooks/hooks.go`, add `"github.com/nathanaday/atlas-obsidian/internal/lint"` to the imports. After `MaxTaskLines`, add:
...
````markdown
```bash
atlas-obsidian lint sensor-triage
atlas-obsidian lint sensor-triage --json
atlas-obsidian lint sensor-triage --strict     # exit 1 when there are findings
```
...
```bash
atlas-obsidian stub sensor-triage                          # every wanted page
atlas-obsidian stub sensor-triage "Bosch" --type entity    # one page, as an entity
```
...
- Replace `` `atlas-obsidian stub` pass it to `txn.Prepare` and `txn.Apply` at once, as`` with `` `atlas-obsidian stub` call `StubPages`, which runs `txn.Prepare` and `txn.Apply` at once, as``.
...
After that section's `atlas-obsidian lint my-project` code block, add:

````markdown
> Create a seed page for each page your links name but nobody has written, as one commit.

```bash
atlas-obsidian stub my-project
```
````
...
```bash
cd $V
ATLAS_OBSIDIAN_BIN=$B ATLAS_OBSIDIAN_HOME=$S/home claude --plugin-dir /Users/nathanaday/SoftwareProjects/atlas-obsidian -p "Lint this wiki and tell me which pages are wanted and which stubs are left to fill." --allowedTools "mcp__plugin_atlas-obsidian_atlas__*,Read,Grep,Glob,Skill"
```
