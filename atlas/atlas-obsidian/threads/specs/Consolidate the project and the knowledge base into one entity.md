...
- `atlas-obsidian upgrade` covers the four migration cases of the design and refuses, by name and with the command to run, where one knowledge base serves several projects. A 3.x project is refused with `ErrSplit` until it runs.
- `make test` passes, and this repository upgrades itself: `atlas_kb/` becomes `atlas/atlas-obsidian/wiki/`.
