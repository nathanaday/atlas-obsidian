...
name: wiki-lint
description: >
  Read-only interpreter for the deterministic wiki linter. Runs the
  lint tool against the project's wiki, validates surprising findings against
...
The parent supplies the project's folder and an optional scope. Fail
...
1. Call `status`, then `lint` (with `exclude` globs when the parent asks for a
   narrower scope). The tools act on the session's project, the
...
4. If the requested scope is narrower than the report, filter only what you
   return; say that the tool scanned the whole wiki.
5. Suggest bounded repairs as proposals. Never apply them, write a report into
   the wiki, or build a plan. Repair is the parent's separate
   operation.
